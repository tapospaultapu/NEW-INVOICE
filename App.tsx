...
import { Product, CartItem, Language, SavedInvoice } from './types';
import { 
  PlusIcon, 
  MinusIcon,
  TrashIcon, 
...
const App: React.FC = () => {
  ...
  const handleCreateInvoice = () => {
    ...
    setHistory(prev => {
      const updated = [newSavedInvoice, ...prev];
  ...
  const handleLoadFromHistory = (inv: SavedInvoice) => {
    setCart(inv.cart);
    setDeliveryCharge(inv.deliveryCharge);
    setAdvancePayment(inv.advancePayment);
  ...
  const handleCopyToClipboard = async () => {
    ...
    text += `${t.timeLabel}: ${invoiceTimestamp.time}\n\n`;
    text += `${t.description}:\n`;

    cart.forEach(item => {
      text += `- ${item.name} (${item.quantity} x ${item.price.toFixed(2)}) = ${t.currency}${(item.price * item.quantity).toFixed(2)}\n`;
    });
    ...
    text += `---------------------------\n`;
    text += `${t.payable}: ${t.currency}${payableAmount.toFixed(2)}\n`;
    text += `${t.advance}: -${t.currency}${numAdvance.toFixed(2)}\n`;
    text += `${t.balanceDue}: ${t.currency}${balanceDue.toFixed(2)}\n`;
    ...
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <header className="relative text-center space-y-1">
          <div className="absolute left-0 top-0 no-print">
            <button 
              ...
              className="p-2 text-gray-500 hover:text-teal-600 hover:bg-white rounded-full transition-all border border-transparent hover:border-gray-100 shadow-sm"
              title={t.history}
            >
          ...
          <div className="absolute right-0 top-0 no-print">
            <button 
              ...
              className="flex items-center gap-2 px-4 py-2 bg-white rounded-full border border-gray-200 shadow-sm hover:shadow-md transition-all text-sm font-bold text-teal-800"
            >
          ...
          {isHistoryOpen && (
            <div className="absolute left-0 top-12 z-50 w-72 bg-white rounded-2xl shadow-2xl border border-gray-100 p-4 animate-in slide-in-from-left-4 text-left no-print">
              <div className="flex items-center justify-between mb-4 pb-2 border-b border-gray-50">
                ...
                <button onClick={() => setIsHistoryOpen(false)} className="text-gray-400 p-1">
                ...
                {history.length === 0 ? (
                  ...
                  history.map((inv) => (
                    <div key={inv.id} className="group relative bg-gray-50 hover:bg-teal-50 rounded-xl p-3 transition-colors border border-gray-100">
                      <div className="text-[10px] text-gray-400 flex items-center justify-between mb-1 italic">
                        <span>{inv.date}</span>
                        <span>{inv.time}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className="text-sm font-bold text-gray-700">{t.currency}{inv.balanceDue.toFixed(0)} {t.balanceDue}</span>
                        <button 
                          onClick={() => handleLoadFromHistory(inv)}
                          className="text-xs bg-white text-teal-600 px-2 py-1 rounded-lg border border-teal-100 font-bold hover:bg-teal-600 hover:text-white transition-all flex items-center gap-1 shadow-sm"
                        >
        ...
        <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 no-print">
          ...
          {isAddingNewProduct && (
            <div className="mb-6 p-4 bg-teal-50 rounded-lg border border-teal-100 animate-in slide-in-from-top-2">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <input 
                  type="text" 
                  placeholder={t.productName}
                  className="px-3 py-2 rounded border border-teal-200 focus:outline-none focus:ring-2 focus:ring-teal-500 text-sm"
                  value={newName}
          ...
          <div className="flex gap-4">
            <select 
              className="flex-1 px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500 bg-gray-50 text-gray-700 font-medium"
              onChange={(e) => {
            ...
            >
              ...
              {products.map(p => (
                <option key={p.id} value={p.id}>{p.name} — {t.currency}{p.price.toFixed(2)}</option>
              ))}
            </select>
          </div>
          ...
          <div className="mt-6">
            ...
            {/* Optimized scrollable cart container */}
            <div className="max-h-[500px] overflow-y-auto border-2 border-dashed border-gray-100 rounded-lg p-2 bg-gray-50/50 custom-scrollbar">
              {cart.length === 0 ? (
                <p className="text-center text-gray-400 py-8 italic text-sm">{t.cartEmpty}</p>
              ) : (
                <ul className="space-y-2">
                  {cart.map((item) => (
                    <li 
                      key={item.uniqueKey} 
                      className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-white px-3 py-2.5 rounded-lg border border-gray-200 shadow-sm animate-in fade-in gap-2"
                    >
                      <div className="flex-1 min-w-0 pr-2">
                        <span className="font-semibold text-gray-800 block text-sm leading-tight break-words">{item.name}</span>
                        <div className="mt-0.5 flex items-center gap-2">
                          <span className="text-[10px] text-gray-400 font-medium uppercase tracking-tight">{t.currency}{item.price.toFixed(2)} {t.each}</span>
                        </div>
                      </div>
                      
                      <div className="flex items-center justify-between w-full sm:w-auto gap-4 sm:gap-6 flex-shrink-0 pt-1 sm:pt-0 border-t sm:border-t-0 border-gray-50">
                        <div className="flex items-center gap-1.5 bg-gray-50 rounded-md border border-gray-200 p-0.5">
                          <button 
                            onClick={() => handleUpdateQuantity(item.uniqueKey, -1)}
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-teal-700 font-bold text-base min-w-[70px] text-right">
                            {t.currency}{(item.price * item.quantity).toFixed(2)}
                          </span>
                          <button 
                            onClick={() => handleRemoveFromCart(item.uniqueKey)}
                            className="text-red-300 hover:text-red-500 p-1.5 transition-colors rounded-md hover:bg-red-50"
                          >
                            <XMarkIcon className="w-5 h-5" />
                          </button>
                        </div>
        ...
        {/* Calculation Inputs and Summary */}
        <section className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 no-print">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-gray-600">{t.deliveryCharge}</label>
              <input 
                type="number" 
                placeholder="0.00"
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-gray-600">{t.advancePayment}</label>
              <input 
                type="number" 
                placeholder="0.00"
                className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500"
            ...
            <div className="space-y-1.5">
              <label className="text-sm font-semibold text-gray-600">{t.discount}</label>
              <input 
                type="number" 
                placeholder="0.00"
                className="w-full px-4 py-3 rounded-lg border border-gray-200 focus:outline-none focus:ring-2 focus:ring-teal-500"
          ...
          <div className="bg-teal-800 text-white rounded-xl p-6 grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="space-y-1">
              <span className="text-teal-200 text-xs font-bold uppercase tracking-wider">{t.subtotal}</span>
              <p className="text-2xl font-bold">{t.currency}{subtotal.toFixed(2)}</p>
            </div>
            <div className="space-y-1">
              <span className="text-teal-200 text-xs font-bold uppercase tracking-wider">{t.payable}</span>
              <p className="text-2xl font-bold">{t.currency}{payableAmount.toFixed(2)}</p>
            </div>
            <div className="space-y-1">
              <span className="text-teal-200 text-xs font-bold uppercase tracking-wider">{t.advance}</span>
              <p className="text-2xl font-bold">{t.currency}{numAdvance.toFixed(2)}</p>
            </div>
            <div className="space-y-1 border-t md:border-t-0 md:border-l border-teal-700/50 pt-4 md:pt-0 md:pl-6">
              <span className="text-teal-200 text-xs font-bold uppercase tracking-wider">{t.balanceDue}</span>
              <p className="text-2xl font-bold text-teal-300 underline underline-offset-4 decoration-2">{t.currency}{balanceDue.toFixed(2)}</p>
            </div>
          </div>
        </section>
        ...
        <div className="flex flex-col md:flex-row gap-4 no-print">
          <button 
            ...
            className="flex-1 bg-teal-600 text-white py-4 rounded-xl font-bold text-lg hover:bg-teal-700 shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2"
          >
          ...
          <button 
            ...
            className="md:w-48 bg-gray-100 text-gray-500 py-4 rounded-xl font-bold text-lg hover:bg-gray-200 transition-all flex items-center justify-center gap-2"
          >
          ...
          <div id="invoice-preview" className="animate-in fade-in zoom-in-95 duration-500 pb-20 print:p-0">
            <div className="bg-white border-2 border-gray-100 rounded-2xl shadow-xl p-8 max-w-2xl mx-auto relative overflow-hidden print:shadow-none print:border-none print:max-w-none">
              ...
              <div className="flex flex-col md:flex-row justify-between items-start mb-10 gap-6">
                <div>
                  <h2 className="text-3xl font-black text-teal-900 leading-tight uppercase tracking-widest">{t.invoice}</h2>
                  <p className="text-gray-400 font-mono text-sm mt-1">#INV-{Math.random().toString(36).substr(2, 6).toUpperCase()}</p>
                </div>
                <div className="text-right">
                  <h3 className="text-xl font-bold text-teal-800">{t.appName}</h3>
                  <p className="text-sm text-gray-500">{t.tagline}</p>
                  <div className="text-xs text-gray-400 mt-2 flex flex-col items-end italic">
                    <span>{invoiceTimestamp.date}</span>
                    <span>{invoiceTimestamp.time}</span>
                  </div>
                </div>
              </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b-2 border-gray-100">
                      <th className="py-4 text-xs font-bold text-gray-400 uppercase tracking-widest">{t.description}</th>
                      <th className="py-4 text-center text-xs font-bold text-gray-400 uppercase tracking-widest px-2">{t.qty}</th>
                      <th className="py-4 text-center text-xs font-bold text-gray-400 uppercase tracking-widest px-2">{t.unitPrice}</th>
                      <th className="py-4 text-right text-xs font-bold text-gray-400 uppercase tracking-widest">{t.total}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {cart.map((item, idx) => (
                      <tr key={idx} className="border-b border-gray-50">
                        ...
                        <td className="py-4 text-center text-gray-600 font-medium px-2">{item.quantity}</td>
                        <td className="py-4 text-center text-gray-600 font-medium px-2">{t.currency}{item.price.toFixed(2)}</td>
                        <td className="py-4 text-right font-bold text-teal-800">{t.currency}{(item.price * item.quantity).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="flex justify-end">
                <div className="w-full md:w-64 space-y-3">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-semibold text-gray-800">{t.currency}{subtotal.toFixed(2)}</span>
                  </div>
                  {numDelivery > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-500">{t.deliveryCharge}</span>
                      <span className="font-semibold text-gray-800">+{t.currency}{numDelivery.toFixed(2)}</span>
                    </div>
                  )}
                  {numDiscount > 0 && (
                    <div className="flex justify-between text-sm text-red-500">
                      <span>{t.discount}</span>
                      <span className="font-semibold">-{t.currency}{numDiscount.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="border-t border-gray-100 pt-3 flex justify-between items-center">
                    <span className="font-bold text-gray-800">{t.payable}</span>
                    <span className="font-black text-xl text-teal-700">{t.currency}{payableAmount.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm pt-2 italic text-gray-500">
                    <span>{t.advancePayment}</span>
                    <span>-{t.currency}{numAdvance.toFixed(2)}</span>
                  </div>
                  <div className="bg-teal-50 p-4 rounded-xl flex justify-between items-center mt-6 print:bg-gray-100">
                    <span className="font-bold text-teal-800">{t.balanceDue}</span>
                    <span className="font-black text-2xl text-teal-900">{t.currency}{balanceDue.toFixed(2)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="mt-12 text-center border-t border-gray-50 pt-6">
                ...
                <div className="flex flex-col gap-3 no-print">
                   <button 
                    onClick={handlePrint} 
                    className="w-full bg-teal-600 text-white py-3 rounded-xl font-bold text-sm hover:bg-teal-700 transition-all flex items-center justify-center gap-2 shadow-sm"
                   >
                   ...
                   >
                     ...
                     {isCopied ? t.copied : t.copyDetails}
                   </button>
                </div>
              </div>
            </div>
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 no-print">
            <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden shadow-2xl animate-in fade-in zoom-in-95">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                <h3 className="font-bold text-lg text-gray-800">{t.manageTitle}</h3>
                  ...
                  {products.map(p => (
                    <li key={p.id} className="py-3 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-gray-500">{t.currency}{p.price.toFixed(2)}</p>
                      </div>
                      <button 
                        onClick={() => handleRemoveProductFromList(p.id)}
      ...
      <footer className="text-center mt-12 py-8 text-gray-400 text-xs no-print">
